...
export class PrismaExceptionFilter implements ExceptionFilter {
  private readonly logger = new Logger('PrismaExceptionFilter');
    ...
    switch (exception.code) {
      case 'P2002':
        res.status(409).json({ statusCode: 409, error: 'Conflict', message: 'resource already exists' });
        return;
      case 'P2025':
        res.status(404).json({ statusCode: 404, error: 'Not Found', message: 'resource not found' });
        return;
      default:
        this.logger.error(`Unmapped Prisma error ${exception.code}: ${exception.message}`);
        res.status(500).json({ statusCode: 500, error: 'Internal Server Error', message: 'database error' });
    }
  }
}
