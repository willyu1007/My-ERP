...
export const CurrentLedgerBook = createParamDecorator((_data: unknown, context: ExecutionContext): LedgerBookEntity => {
  const req = context.switchToHttp().getRequest<AuthedRequest>();
  if (!req.ledgerBook) throw new ForbiddenException('no ledger scope (LedgerScopeGuard must run first)');
