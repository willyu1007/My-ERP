...
describe.skipIf(!PG_AVAILABLE)(
  'Postgres RLS — contract (ledger-scoped) + contractId dimension',
  () => {
    beforeAll(async () => {
    ...
    it('creates, lists, and isolates contracts by ledger', async () => {
      const c = await withLedgerScope(LB_A, (tx) =>
        createContractTx(tx, contractInput('HT-2026-001')),
      );
      ...
      expect((await withLedgerScope(LB_A, (tx) => getContractTx(tx, c.id)))?.code).toBe(
        'HT-2026-001',
      );
    ...
    it('optimistic update bumps version; a stale version conflicts (null)', async () => {
      const c = await withLedgerScope(LB_A, (tx) =>
        createContractTx(tx, contractInput('HT-2026-002')),
      );
      ...
      expect(
        await withLedgerScope(LB_A, (tx) =>
          updateContractTx(tx, c.id, { expectedVersion: 0, status: 'closed' }),
        ),
      ).toBeNull();
    });
    ...
    it('finds vouchers + payments linked by contractId (the timeline dimension)', async () => {
      const c = await withLedgerScope(LB_A, (tx) =>
        createContractTx(tx, contractInput('HT-2026-003')),
      );
