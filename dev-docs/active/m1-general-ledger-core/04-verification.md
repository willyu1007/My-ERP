...
### P5 — 期初建账 — 2026-06-10（本机 PG17 :5432）

| 验证项 | 命令 | 结果 |
|---|---|---|
| 类型检查 | `pnpm -r typecheck` | ✓ 9/9 |
| 单测 + 集成 | `pnpm test` | ✓ 78 passed（`openingBalanceError` 3 单测 + `opening-balance.integration` 3：整组替换/派生纳入期初/`WITH CHECK` 阻跨账套）|
| Lint / build | `pnpm lint · build` | ✓ 无告警 / Done |
| **端到端 HTTP** | 本机 PG（全迁移）+ 两会计 + 非特权角色起 api | ✓ ① PUT 期初**不平 → 400**（启用期借=贷）② PUT 平衡 → 200 ③ GET 启用期=2026-06、期初 2 项 ④ 过账后 trial-balance：balanced opening/closing；**4001 期初贷80000 + 本期贷5000 = 期末贷85000**（基线累计）⑤ 明细账 4001 期初80000贷→期末85000贷 ⑥ 使用后 PUT 期初 → **400**（须在使用前）；日志 0 错误 |
