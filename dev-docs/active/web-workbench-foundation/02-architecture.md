...
apps/web/                   财务语义
  ...
  src/lib/finance/          VM 类型 + fixtures fallback + API-first data-source + scene-config + format
...
- **数据流（API 优先）**：页面（server, force-dynamic）→ `data-source`（配置后走 `/v1`；未配置时 fixture fallback 只用于本地只读预览）→ VM → client 组件。**VM 形状是单一事实源**，避免页面和接口形成双轨。
