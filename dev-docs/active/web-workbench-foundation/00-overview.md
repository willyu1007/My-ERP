...
## Status
- **W2a done / W2b next** —— W0（套件移植 + governance）、W1（ERP 总览 + 凭证三页）、W2a（会计科目树）均完成并验证（见 04-verification）。在 `main` 上实现（沿用 P0a：不另开分支）。
- 架构修正（W1）：首页是 **ERP 整体**（模块化平台，财务是第一个模块），财务模块按 `/finance/` 命名空间；凭证二级导航按 **工作流动作**（制单/审核/过账/红冲）组织。
- W2a：`/finance/accounts` 占位 → 科目树（多级 + 辅助核算 + 启停，按类别分段）；`AccountVM` 扩展 parentCode/level/auxTypes/active；凭证分录改挂末级科目；制单下拉仅末级且启用。
- Next：W2b —— 账簿（试算平衡 + 总账/明细账，从已过账凭证派生）；之后 W2c 角色工作台/待办。随 M1 P1–P5 把 `data-source` 从 fixtures 切到真实 `/v1`。
